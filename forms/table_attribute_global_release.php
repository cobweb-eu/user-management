...
// Add default values
$v->setDefault('title', 'Global attribute release approval');
...
// Load entities
$entities = loadGlobalEntity($get['user_name'],'1');
...
<div class='panel panel-primary'>
  <div class='panel-heading'>
    <h3 class='panel-title'><i class='fa'></i> {$get['title']}</h3>
  </div>
...
if (count($entities) > 0) {
    ...
    $response .= $tb->render(false);
    $response .= "</div>";
} else {
    $response .= "<div class='alert alert-info'>No global approval found.</div>";
}
